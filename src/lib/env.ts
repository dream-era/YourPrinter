...
function getServerEnv() {
  ...
  if (!serverParsed) {
    serverParsed = serverEnvSchema.parse({
...
export const env = new Proxy({ ...publicParsed } as z.infer<typeof publicEnvSchema> & z.infer<typeof serverEnvSchema>, {
  get(target, prop) {
    if (prop in target) {
    ...
    if (prop in sEnv) {
      return sEnv[prop as keyof typeof sEnv];
    }
    return undefined;
  },
});
