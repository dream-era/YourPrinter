 ...
 * Key ID + Key Secret (decrypted from shop_payment_settings). Every
 * payment operation goes through getShopRazorpayClient(shopId) — there is
 * no global/platform Razorpay client for taking student payments.
 */
...
export interface ShopPaymentCredentials {
  keyId: string;
  keySecret: string;
  webhookSecret: string | null; // null if the owner hasn't set one yet — see migration 0016
 ...
 * or verification failed — callers (order creation) should catch this and
 * surface a clear "this shop can't accept payments yet" error.
 */
export async function getShopPaymentCredentials(
  ...
  if (data.razorpay_webhook_secret_enc) {
