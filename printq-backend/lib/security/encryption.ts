 ...
 *
 * Master key comes from process.env.MASTER_ENCRYPTION_KEY — a 32-byte key,
 ...
 *
 * Rotation note: if you ever rotate MASTER_ENCRYPTION_KEY, every row in
...
function getMasterKey(): Buffer {
  ...
  if (!key) {
    throw new Error(
      "MASTER_ENCRYPTION_KEY is not set. Generate one with `openssl rand -base64 32` and add it to your environment."
    );
  }
  const buf = Buffer.from(key, "base64");
  if (buf.length !== 32) {
    throw new Error(
      `MASTER_ENCRYPTION_KEY must decode to exactly 32 bytes, got ${buf.length}. Regenerate with \`openssl rand -base64 32\`.`
    );
  }
  return buf;
}
