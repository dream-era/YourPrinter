...
export async function POST(req: NextRequest) {
  ...
  for (const row of unsettledRows ?? []) {
    const entry = byShop.get(row.shop_id) ?? { total: 0, ids: [], earliest: row.created_at };
  ...
  for (const [shopId, { total, ids, earliest }] of byShop.entries()) {
    ...
    try {
      ...
      const paymentLink = await platformClient.paymentLink.create({
        amount: total,
        currency: "INR",
        description: `YourPrinter commission — ${shop?.name ?? shopId}`,
        notes: { settlement_batch_id: batch.id, shop_id: shopId },
        // reminder_enable: true, // uncomment if your Razorpay plan supports payment link reminders
      });
